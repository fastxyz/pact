...
```
LOOP_DONE_claude-code_<short-sha>
...
5. Print to user: "Merge gate (CONTRACT §7) needs a clean marker from a different vendor on HEAD `<short-sha>`. Switch to another vendor's CLI and run `/review <PR>` (or `/loop <PR>`)."

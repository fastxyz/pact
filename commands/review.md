...
7. Aggregate findings into per-lane per-severity counts, then sum CQ+SP+TC into aggregate P0/P1/P2/P3 totals.
8. Emit the marker. The marker first line MUST include the aggregate P0/P1/P2/P3 totals immediately after the marker title, e.g. `REVIEW_FINDINGS_<vendor>_R<N>_<sha> P0=<n> P1=<n> P2=<n> P3=<n>`. The marker body MUST include the `Existing markers on HEAD <sha>:` field (see CONTRACT §5) enumerating every prior marker on the current HEAD. These are hard schema requirements, not optional: the first line lets humans scan severity immediately, and the existing-marker field forces the reviewer to derive the closing line from observed state rather than from a template.
